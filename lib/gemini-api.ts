...
function buildPrompt(scenario: MedicalScenario): string {
  ...
  const versionType = shortVersion ? 'КОРОТКАЯ (без вопросов пострадавшему)' : 'ПОЛНАЯ (с вопросами и вариациями)'
...
- Версия отыгровки: ${versionType}

СТРОГИЕ ПРАВИЛА ОФОРМЛЕНИЯ:

1. КОМАНДА /me (действия от 3-го лица):
   - Пишется с МАЛЕНЬКОЙ буквы (перед ней стоит имя персонажа)
   - ⚠️ СТРОГО ЗАПРЕЩЕНО ставить точку в конце /me команды! ⚠️
   - НИКОГДА не пиши: "/me достал перчатки." ❌
   - ВСЕГДА пиши: "/me достал перчатки" ✅
...
7. ВАРИАЦИИ С ОТВЕТАМИ ПОСТРАДАВШЕГО:
   ${shortVersion 
     ? `
   ⚠️ КОРОТКАЯ ВЕРСИЯ - БЕЗ ВОПРОСОВ И ВАРИАЦИЙ ⚠️
   
   СТРОГО ЗАПРЕЩЕНО:
   - Вопросительные /do: "/do Пульс присутствует?" ❌
   - Инструкции: "/b Ответ в /do Да." ❌
   - Вариации: "— Если пульс есть:" ❌
   - Любые /b команды ❌
   
   ПРАВИЛЬНО (короткая версия):
   /me приложил два пальца к сонной артерии
   /me обнаружил слабый пульс
   /do Пульс слабый, но прощупывается.
   /me продолжил осмотр пострадавшего
   /me достал стерильную салфетку
   /me прижал салфетку к ране
   /do Кровотечение уменьшается.
   /me наложил давящую повязку
   
   Используй ТОЛЬКО утвердительные /do:
   - "/do Кровотечение уменьшается." ✅
   - "/do Пульс прощупывается." ✅
   - "/do Перелом зафиксирован." ✅
   ...
   ОБЯЗАТЕЛЬНО после КАЖДОГО вопросительного /do писать:
   1. Инструкцию для пострадавшего: /b Ответ в /do Да. (Или /do Нет.)
   2. ДВА ПОЛНЫХ варианта развития событий`
   }
   ...
   ЭТО ОБЯЗАТЕЛЬНО ДЛЯ ВСЕХ ВОПРОСОВ:
   - /do Пульс присутствует? → ДВА варианта (есть/нет)
   - /do Дыхание присутствует? → ДВА варианта (есть/нет)
   - /do Кровотечение уменьшается? → ДВА варианта (да/нет)
   - /do Кровотечение остановилось? → ДВА варианта (да/нет)
   - /do Пострадавший в сознании? → ДВА варианта (да/нет)
   - /do Пульс прощупывается? → ДВА варианта (да/нет)
   - /do Повязка наложена? → НЕ НУЖНА вариация (это утверждение, не вопрос к пострадавшему)
   
   СТРУКТУРА ОТЫГРОВКИ:
   1. Действие
   2. Вопрос /do
   3. ОБЯЗАТЕЛЬНО два варианта с "— Если..."
   4. Следующее действие
   5. Следующий вопрос /do
   6. ОБЯЗАТЕЛЬНО два варианта с "— Если..."
   И так далее для КАЖДОГО вопроса!
