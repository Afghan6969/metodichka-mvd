...
export function GibddBindsPage() {
 ...
 <div className="group flex items-start gap-3 p-4 rounded-xl bg-gradient-to-r from-primary/5 to-primary/10 border-2 border-primary/20 hover:border-primary/40 hover:shadow-lg transition-all">
 <div className="w-10 h-10 bg-primary rounded-lg flex items-center justify-center shadow-md group-hover:scale-110 transition-transform">
 ...
 return (
 <div key={sectionIndex} className="rounded-xl border-2 border-primary/20 bg-card overflow-hidden hover:border-primary/40 transition-all">
 <button
 onClick={() => toggleSection(sectionIndex)}
 className="w-full px-6 py-4 bg-gradient-to-r from-primary/10 to-primary/5 hover:from-primary/15 hover:to-primary/10 transition-all flex items-center justify-between group"
 >
 ...
 <ChevronDown className={`h-5 w-5 text-primary transition-transform duration-300 ${isExpanded ? 'rotate-180' : ''}`} />
 </div>
 </button>
 <div className={`overflow-hidden transition-colors duration-200 ease-in-out ${isExpanded ? 'max-h-[5000px] opacity-100' : 'max-h-0 opacity-0'}`}>
