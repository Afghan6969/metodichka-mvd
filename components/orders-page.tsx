...
Время: [Время] (МСК)
Обязательная явка всего личного состава [Фракция].
...
Принят сотрудник: [Ник]
Присвоено звание [Звание], назначена должность [Должность]
Желаем успешной службы и профессионального роста.
...
Сотрудник [Ник] уволен из [Фракция] по собственному желанию.
...
Новый никнейм: [Новый никнейм]`
  ...
  const extractPlaceholders = (content: string): string[] => {
    ...
    // Удаляем первую строку с заголовком [ Должность | Никнейм ]
    const contentWithoutHeader = content.replace(/^\[\s*Ваша должность\s*\|\s*Ваш никнейм\s*\]\n?/m, '')
    
    // Извлекаем плейсхолдеры в формате [Плейсхолдер]
    const regex = /\[([^\]]+)\]/g
    const matches = contentWithoutHeader.match(regex) || []
  ...
  const openEditor = (order: Order) => {
    ...
    placeholders.forEach(placeholder => {
      // Все плейсхолдеры в формате [Плейсхолдер]
  ...
  const applyReplacements = (content: string): string => {
    ...
    // Замена должности и никнейма в заголовке [ Должность | Никнейм ]
    if (position && nickname) {
      result = result.replace(/^\[\s*Ваша должность\s*\|\s*Ваш никнейм\s*\]/m, `[ ${position} | ${nickname} ]`)
    } else if (position) {
      result = result.replace(/Ваша должность/g, position)
    }
    if (nickname) {
      result = result.replace(/Ваш никнейм/g, nickname)
    }
...
Текущее дисциплинарное состояние: УП — ${emp.disciplineUp}/5 | П — ${emp.disciplineP}/5 | В — ${emp.disciplineV}/3.`
    ...
    Object.entries(formData).forEach(([placeholder, value]) => {
      if (value) {
        // Если плейсхолдер начинается со скобки - заменяем как есть
        if (placeholder.startsWith('[')) {
          result = result.replaceAll(placeholder, value)
        } else {
          // Для плейсхолдеров без скобок (из заголовка) заменяем напрямую
          result = result.replaceAll(placeholder, value)
        }
      }
    })
              ...
              <div className="space-y-6">
                ...
                <div className="bg-red-950/20 backdrop-blur-sm border border-red-900/30 rounded-2xl p-5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <Label className="text-sm text-red-100/80 mb-2 block font-medium">Ваша должность</Label>
                      <Input
                    ...
                    <div>
                      <Label className="text-sm text-red-100/80 mb-2 block font-medium">Ваш никнейм</Label>
                      <Input
